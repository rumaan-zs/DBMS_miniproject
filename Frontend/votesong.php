...
if(!isset($_POST['song'])){
	...
	$checkIP = mysqli_query($link,"SELECT * FROM tblip WHERE ip = '$ip' AND time > '$out'") or die (mysqli_error($link));
	if(mysqli_num_rows($checkIP) >=1){
		echo "<script>alert('Opps..You cand only vote once a day!');window.location.href='Songs.php';</script>";	
	}else{
	$insertIP = mysqli_query($link,"INSERT INTO tblip(ip,time) VALUES ('".$ip."','".$time."')") or die(mysqli_error($link));
	$updatepoints = mysqli_query($link,"UPDATE tblsongs SET songpoints = songpoints + 1 WHERE id = '".$song."'") or die (mysqli_error($link));
